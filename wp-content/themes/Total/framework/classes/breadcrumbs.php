	...
	class WPEX_Breadcrumbs {
		...
		private function get_post_parents( $post_id = '', $path = '' ) {
			...
			while ( $post_id ) {
				...
				$parents[]  = '<a href="'. esc_url( get_permalink( $post_id ) ) .'" title="'. esc_attr( get_the_title( $post_id ) ) .'" itemprop="url">'. get_the_title( $post_id ) .'</a>';
		...
		private function get_term_parents( $term = '' ) {
			...
			if ( is_taxonomy_hierarchical( $taxonomy ) && $term->parent != 0 ) {
				...
				while ( $term->parent != 0 ) {
					...
					$parents['parent_term_'. $count ] = '<a href="'. esc_url( get_term_link( $term, $taxonomy ) ) .'" title="'. esc_attr( $term->name ) .'" itemprop="url">'. $term->name .'</a>';
