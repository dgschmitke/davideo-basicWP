		...
		if ( $filter_terms ) {
			...
			if ( isset( $_GET[$filter_url_param] ) ) {
				...
				if ( ! is_numeric( $filter_active_category ) ) {
					$get_term = get_term_by( 'name', $filter_active_category, $atts['taxonomy'] );
					if ( $get_term ) {
						...
						if ( 'true' == $title
							...
							<div class="staff-entry-details entry-details wpex-clr<?php if ( $content_css ) echo ' '. $content_css; ?>" <?php echo $content_style; ?>>
								...
								if ( 'true' == $title ) : ?>
									...
									<<?php echo $title_tag; ?> class="staff-entry-title entry-title"<?php echo $heading_style; ?>>
									...
									</<?php echo $title_tag; ?>><!-- .staff-entry-title -->
